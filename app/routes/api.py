...
from app.exceptions import GreenPrintError, ValidationError
...
def _finalize(payload: dict, language: str, started_at: float):
...
@api_bp.errorhandler(GreenPrintError)
def handle_domain_error(error: GreenPrintError):
    """Stable JSON error contract for every domain failure."""
...
def track_activity():
    ...
    payload = {
        ...
        "motivation_tone": motivation["tone"],
    }
...
def simulate_scenario():
    ...
    projection["narrative"] = services.engine.draft_simulation_narrative(
        scenario, projection["weekly_saving_kg"]
    )
